...
const generateCompoundVariants = (sides: Array<Sides>) => {
  return sides.map((side) => ({
    ...
    className:
      side === "top"
        ? "top-2 inset-x-2 rounded-lg ring border-b-0"
        : side === "bottom"
          ? "bottom-2 inset-x-2 rounded-lg ring border-t-0"
          : side === "left"
            ? "left-2 inset-y-2 rounded-lg ring border-r-0"
            : "right-2 inset-y-2 rounded-lg ring border-l-0",
  }))
}

const sheetContentStyles = tv({
  base: "fixed z-50 grid gap-4 border-muted-fg/20 bg-overlay text-overlay-fg shadow-lg transition ease-in-out",
  variants: {
    isEntering: {
