      ...
      <Form className="space-y-12">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-[26rem_1fr]">
          ...
          <div className="flex gap-6">
            <div className="group relative size-full shrink-0 overflow-hidden rounded-lg sm:sm:size-64">
              <img
                src={fileInfo ? fileInfo.previewUrl : "/images/placeholder.jpg"}
                alt={fileInfo ? fileInfo.name : "No image"}
                className={twJoin(
                  "aspect-square size-full rounded-lg border-gray-400 object-cover object-center duration-300 sm:sm:size-64",
                  fileInfo && "group-hover:scale-110",
                )}
              />
              ...
              <FileTrigger onSelect={handleSelect} acceptedFileTypes={["image/*"]}>
                <PrimitiveButton
                  className={twJoin(
                    "absolute inset-0 z-20 flex items-center justify-center transition duration-200",
                    fileInfo && "opacity-0 group-hover:opacity-100",
                  )}
                >
                  <span className="z-30 flex items-center gap-x-1 rounded-full bg-white px-2 py-1.5 font-medium text-gray-900 text-xs">
                    ...
                    {fileInfo ? "Replace image" : "Upload image"}
                  </span>
                </PrimitiveButton>
              </FileTrigger>
              ...
              <div
                className={twJoin(
                  "absolute inset-0 bg-gray-950/50",
                  fileInfo && "hidden group-hover:block",
                )}
              />
            </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-[26rem_1fr]">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 ">
            ...
            />
            <NumberField minValue={1} label="Stock" placeholder="Enter stock quantity" />
          </div>
        </div>
